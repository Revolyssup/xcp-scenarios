...
- Cluster role: **unknown** (no Service "backend" in namespace "backend-c1" and no XCP tier-1 CR names this hostname — likely either a tier-1 gateway configured outside XCP (check 01-envoy/<pod>/config_dump.json) or the wrong namespace/hostname for this cluster (cluster also has tier-1 CRs [Gateway xcp-system/transit-mid-gw] — none of them name this hostname))
- Collected: 2026-06-01T10:50:09Z
...
- `xcp-config` (03-xcp-config): ok, 13 file(s)
